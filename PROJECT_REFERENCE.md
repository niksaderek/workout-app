...
Full reference for Workout Pro (`index.html`). Read on demand — not auto-loaded. Loaded by CLAUDE.md pointer.
...
### Bodyweight-Aware Recommendation Override
The proactive recommendation engine (`getSmartWeightSuggestion`) reads from the `bodyWeight` IndexedDB store via `getBodyWeightAt(dateISO)` (nearest-date lookup over `bodyWeightHistory` state, loaded on mount and refreshed after `handleSaveBodyWeight`). `getExerciseHistory` attaches `bodyWeight` + `isBodyweight` per entry, **preferring `workout.bodyWeightSnapshot`** (set in `finishWorkout`) over the nearest-date lookup — the global store is sparse in practice, so the snapshot is the accurate source going forward.

Override placement: between raw-metric plateau detection and the deload branch. Reclassifies a raw plateau as `progress` when:
- **Weighted lift:** `latest.bestE1RM / latestBW > earlierBestEntry.bestE1RM / earlierBW × 1.01` (got stronger relative to bodyweight).
- **Bodyweight exercise** (pull-up/push-up/chin-up/dip via top-level `isBodyweightExercise`): reps held or grew AND BW dropped >1% (same work at lower effective load).

There is also a `rep-up` branch (after plateau handling, before existing rep-range branch) that praises an added rep at same-or-heavier weight when the planned target isn't yet hit.

### Strength-Standards Main-Lift Whitelist
`categorizeExercise` matches exercise names against `EXERCISE_CATEGORIES` using **case-insensitive EXACT match after stripping a trailing `(…)` parenthetical**. Only competition-style barbell variants count toward the main-lift 1RM totals and Strength Standards bars (Squat / Bench Press / Deadlift / Overhead Press). Variants like Leg Press, Hack Squat, Bulgarian Split Squat, Goblet Squat, Romanian Deadlift, Dumbbell Bench, Incline/Decline Bench, Push Press, Arnold Press, DB Shoulder Press are **deliberately excluded** — they still appear in Exercise Progress / Estimated 1RM lists, just not in Strength Standards (which is calibrated on competition lifts). Names like `"Overhead Press (OHP)"`, `"Bench Press (Comp)"`, `"Squat (Pause)"` still match because of the parenthetical strip.

### Edit-History Exercise Rename
The edit-history view uses `<ExerciseInput>` (not a static `<h3>`) for the exercise name, wired through `updateHistoryExerciseName`. This lets users correct mis-named exercises in past workouts (e.g. a set logged as Back Squat that was actually Leg Press).

### Exercise Measurement Units & Type Classification
Each exercise resolves to a measurement unit via the single helper `getExerciseUnit(exercise)` → `'reps' | 'seconds' | 'meters'`, backed by `UNIT_CONFIG` (per-unit `label`, `placeholder`, `hasWeight`, `countsAsReps`, `countsAsVolume`). An explicit per-exercise override `exercise.unit` (set in the template editor; copied into history by `startWorkout`) wins; otherwise the unit is auto-detected by name regex (farmer/carry/walk/sled/yoke → meters; plank/hold/wall sit/hang → seconds; else reps).

- The logged value stays in the existing `set.reps` field — reinterpreted as reps/seconds/meters, never renamed (this is what makes the feature zero-migration).
- `seconds`/`meters` exercises are excluded from rep totals, volume, and 1RM and counted separately — exactly how core exercises were treated before. This replaced the old duplicated `name.toLowerCase().includes('core'|'plank'|'hanging'|'rollout')` checks across ~8 sites; all of them now route through `UNIT_CONFIG[getExerciseUnit(x)]` (`countsAsVolume` for volume/1RM gates, `countsAsReps` for rep-count splits, `!== 'reps'` for pure skip).
- All units keep an optional weight field (`hasWeight: true`) so weighted planks / loaded carries can be recorded; that weight is still excluded from volume/1RM.
- `formatPlannedTarget(plannedReps, label)` appends the unit label only when `plannedReps` is a bare number/range, since existing templates store free-text like "40 m" or "To failure".
- `isBodyweightExercise` (pull-up/push-up) remains a separate concern and is intentionally NOT part of the unit system.
